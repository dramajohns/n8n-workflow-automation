...
- **n8n Instance**: n8n self-hosted (localhost:5678)
- **MCP Server**: n8n-mcp (czlonkowski/n8n-mcp)
- **Skills**: n8n-skills (czlonkowski/n8n-skills)

## Setup & Installation

This project requires configuration of both the n8n MCP server and n8n skills.

### Quick Setup (Windows)
1. **Configure MCP Server**: See `docs/setup/MCP_CONFIGURATION.md`
   - Location: `%APPDATA%\Claude\claude_desktop_config.json`
   - Required: N8N_API_URL and N8N_API_KEY environment variables
   - Installation: NPX (recommended) or Docker

2. **Install n8n Skills**: See `docs/setup/SKILLS_INSTALLATION.md`
   - Run: `/plugin install czlonkowski/n8n-skills`
   - Location: `.claude/skills/` in this project

3. **Complete Windows Setup**: See `docs/setup/WINDOWS_SETUP.md`
   - Configure environment variables
   - Setup backup automation
   - Test MCP tools

### Project Structure
- `workflows/dev/` - Development and testing workflows
- `workflows/production/` - Production-ready workflows
- `workflows/templates/` - Reusable templates and patterns
- `scripts/` - PowerShell automation scripts (backup, export, validate)
- `docs/` - Complete setup and usage documentation
...
- See `docs/guides/VERSION_CONTROL.md` for Git workflow

### Self-Hosted n8n Configuration
For production-ready self-hosted setup, see `docs/architecture/SELF_HOSTED_SETUP.md`:
- PostgreSQL database (not SQLite)
- Queue Mode with Redis
- Proper resource allocation (4+ vCPUs for production)
